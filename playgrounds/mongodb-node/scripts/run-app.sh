#!/usr/bin/env bash
# Run the MongoDB Node.js native driver demo API against local DocumentDB.
set -euo pipefail
...
echo "Installing app dependencies (express, mongodb)..."
(cd "$APP_DIR" && npm install --omit=dev --no-audit --no-fund >/dev/null)

echo ""
echo "=== MongoDB Node.js driver demo API running locally ==="
